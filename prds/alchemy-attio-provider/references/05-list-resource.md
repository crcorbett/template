...
## Resource Definition (`src/attio/list/list.ts`)
...
- `stables: ["listId", "apiSlug", "parentObject"]`
- `diff`: parentObject change → replace; name → update
- `read`: getList by listId or apiSlug; fallback scan listLists by name
- `create`: scan listLists for name match
- `update`: updateList with name
- `delete`: hard delete via deleteList + catch NotFoundError
