 ...
 * Attio API Client
 *
 * Main client for making typed API calls to Attio.
 * Uses Bearer token authentication with the Attio API key.
 */
...
interface OperationInit {
  ...
  readonly parseResponse: (
    response: Response
  ) => Effect.Effect<S.Schema.Type<Operation["output"]>, AttioErrorType>;
}
...
/**
 * Extract a query parameter value from a "next" URL.
 ...
 *
 * Uses `operation.pagination` metadata to generically handle url-based,
 * offset-based, and cursor-based pagination. Returns a callable with two
 * additional methods:
