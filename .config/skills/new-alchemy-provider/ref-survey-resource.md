# Reference: Survey Resource (Input<T> + String UUID IDs)

Source: `packages/alchemy-posthog/src/posthog/surveys/survey.ts`

This resource demonstrates two patterns NOT shown in the FeatureFlag reference:
1. **`Input<T>`** for cross-resource references (`linkedFlagId?: Input<number | null>`)
2. **String UUID IDs** (`id: string` instead of `id: number`)
3. **`Input.Resolve<Props>`** on the Attrs generic to resolve lazy references
...
export interface SurveyProps {
   ...
   * NOTE: Wrapped in Input<T> because the value may come from another
   * resource's output attribute (e.g. myFeatureFlag.id).
   */
 ...
 * NOTE: Uses Input.Resolve<SurveyProps> because Props contains Input<T> fields.
 */
export interface SurveyAttrs<_Props extends Input.Resolve<SurveyProps> = Input.Resolve<SurveyProps>> {
  /**
   * Server-generated UUID (stable). String, not number.
   */
  id: string;
